...
from metaworld import MT1, MT10
...
def parse_args():
    ...
    parser.add_argument('--env_name', type=str, default='reach-v2', help='environment to run')
...
def main():
    ...
    save_dir = os.path.join(args.root_dir, args.env_name+'_expert')
    ...
    for task in mt1.train_tasks:
        ...
        while count < minimal_episode_length and not done:
            count += 1
            a = p.get_action(obs)
        ...
        print('Episode: {}, Success: {}'.format(episode_idx, info["success"]))
        save_path = os.path.join(save_dir, f'{episode_idx}')
        os.makedirs(save_path, exist_ok=True)
        for i, obs in enumerate(img_list):
            img_path = os.path.join(save_path, f'{i}.png')
            # save the np array as png
